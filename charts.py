...
def generate_income_vs_expenses(transactions, period):
    ...
    try:
        ...
        # ✅ Only add text labels for bars with height > 0
        ...
        print(f"Error generating income vs expenses chart: {e}")
...
def generate_spending_by_category(transactions, period):
    ...
    try:
        ...
        for transaction in expenses:
            category_totals[transaction.get('category', 'Uncategorized')] += transaction['amount']
        ...
        print(f"Error generating spending by category chart: {e}")
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.text(0.5, 0.5, f'Error generating chart: {str(e)}', 
                ha='center', va='center', fontsize=12)
        ax.axis('off')
...
def generate_trend_over_time(transactions, period):
    ...
    try:
        ...
        # ✅ Fix fill_between condition
        positive = [n >= 0 for n in net_totals]
        negative = [n < 0 for n in net_totals]
        ...
        print(f"Error generating trend over time chart: {e}")
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.text(0.5, 0.5, f'Error generating chart: {str(e)}', 
                ha='center', va='center', fontsize=12)
        ax.axis('off')
...
def generate_all_charts(transactions, period):
    ...
    # ✅ Return the paths to the generated charts
    return {
        'income_vs_expenses': os.path.join(CHARTS_DIR, 'income_vs_expenses.png'),
        'spending_by_category': os.path.join(CHARTS_DIR, 'spending_by_category.png'),
        'trend_over_time': os.path.join(CHARTS_DIR, 'trend_over_time.png')
    }
