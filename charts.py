...
def safe_get(transaction, key, default=None):
    """Safely get a value from either a dict or sqlite3.Row object"""
    try:
        if hasattr(transaction, 'keys'):  # It's a dict
            return transaction.get(key, default)
        else:  # It's a sqlite3.Row or similar
            return transaction[key] if key in transaction.keys() else default
    except:
        return default
        ...
        for transaction in transactions:
            try:
                ...
                if 0 <= days_diff < 7:
                    ...
                    txn_type = transaction['type'] if 'type' in transaction.keys() else transaction.get('type')
                    if txn_type == 'income':
...
def generate_spending_by_category(transactions, period):
    try:
        ...
        for t in transactions:
            txn_type = t['type'] if 'type' in t.keys() else t.get('type')
            if txn_type == 'expense':
                expenses.append(t)
        ...
        for transaction in expenses:
            category = transaction['category'] if 'category' in transaction.keys() else transaction.get('category', 'Uncategorized')
